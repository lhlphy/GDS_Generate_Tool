...
write_gds_library(gdsl,'!MetaSurface_rect4.gds');
...
function [cell1] = create_meta2(cell1,layerMap,index_X,index_Y,Px,Py,a,b,Rot,lam_eff,Phi_grad)
    dy = 0.5; %y方向偏移
    Pos0 = [Px*(index_X-1)+ a/2*cosd(Rot) + b/2*sind(Rot) + mod((index_Y-1)*Py*Phi_grad,2*pi)/(2*pi)*lam_eff, -Py*(index_Y-1)+ a/2*sind(Rot) - b/2*cosd(Rot)];  %单元格左上角点
